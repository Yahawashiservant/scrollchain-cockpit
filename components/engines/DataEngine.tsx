      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-left text-xs font-mono">
          <thead className="bg-white/5 sticky top-0 backdrop-blur-md">
            <tr><th className="p-3 text-gray-400">ID</th><th className="p-3 text-gray-400">HASH</th><th className="p-3 text-gray-400">VALUE</th><th className="p-3 text-gray-400">STATUS</th></tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {rows.map((r, i) => (
              <tr key={i} className="hover:bg-white/5 transition-colors">
                <td className="p-3 text-blue-300">{r.id}</td><td className="p-3 text-gray-500">{r.hash}</td><td className="p-3 text-white">{r.val}</td><td className="p-3 text-green-400">{r.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
